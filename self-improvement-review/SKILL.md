...
- **`collect_sessions.py` 只发现真实 Claude Code session JSONL**；include 范围限定于 `projects/**/*.jsonl`、`sessions/**/*.jsonl`、`history.jsonl`；exclude 插件/cache/skills/node_modules 源码及 `.js/.ts/.sh/.map/references/*.md`；不读取 transcript 全文，不提取 quote，除非用户明确要求进入 session review
